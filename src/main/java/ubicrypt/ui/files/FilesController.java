...
public class FilesController implements Initializable, ApplicationContextAware {
    ...
    }).collect(toList())).subscribe(empty(), err -> {
        log.error(err.getMessage(), err);
    }, () -> {
    });
    private final Consumer<Path> folderAdder = fromFolder -> {
        DirectoryChooser dc = new DirectoryChooser();
        dc.setInitialDirectory(basePath.resolve(fromFolder).toFile());
        dc.setTitle("Select Folder to Save");
        ofNullable(dc.showDialog(stage)).ifPresent(folder -> {
            try {
    ...
    private synchronized TreeItem<ITreeItem> addFiles(final Iterator<Path> it, final Path rootPath, final TreeItem<ITreeItem> root, final LocalFile file) {
        ...
        if (Files.isRegularFile(resolvedPath)) {
            ...
            root.getChildren().sort((iTreeItemTreeItem, t1) -> iTreeItemTreeItem.getValue().getLabel().compareTo(t1.getValue().getLabel()));
            return fileItem;
        }
        ...
        final TreeItem<ITreeItem> fileItem = new TreeFolderItem(new FolderItem(path, event -> fileAdder.accept(resolvedPath), event -> folderAdder.accept(resolvedPath)));
        root.getChildren().add(fileItem);
        root.getChildren().sort((iTreeItemTreeItem, t1) -> iTreeItemTreeItem.getValue().getLabel().compareTo(t1.getValue().getLabel()));

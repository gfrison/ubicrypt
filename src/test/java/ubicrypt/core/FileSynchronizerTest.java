...
public class FileSynchronizerTest {
  ...
  public void packFiles() throws Exception {
    ...
    final List<FileProvenience> collect =
        map.get(uuid).stream().filter(fp -> fp.getOrigin().isLocal()).collect(Collectors.toList());
    assertThat(collect).hasSize(1);
    assertThat(
            map.get(uuid)
                .stream()
                .filter(fp -> !fp.getOrigin().isLocal())
  ...
  public void conflictsMix() throws Exception {
    ...
    final Multimap<UUID, FileProvenience> input =
        ImmutableMultimap.of(uuid, fp1, uuid, fp3, uuid1, fp2);
    final Multimap<UUID, FileProvenience> conflicts = FileSynchronizer.conflicts(input);
    assertThat(conflicts.keySet().size()).isEqualTo(1);

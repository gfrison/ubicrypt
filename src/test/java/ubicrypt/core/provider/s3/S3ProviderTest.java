...
public class S3ProviderTest {
    ...
    public void name() throws Exception {
        ...
        test.setConf(conf);
        assertThat(test.init(Utils.deviceId()).toBlocking().first()).isEqualTo(ProviderStatus.initialized);
        String id = test.post(new ByteArrayInputStream("ciao".getBytes())).toBlocking().first();
        assertThat(IOUtils.toString(test.get(id).toBlocking().first(), Charset.defaultCharset())).isEqualTo("ciao");
        assertThat(test.put(id, new ByteArrayInputStream("ciao2".getBytes())).toBlocking().first()).isTrue();
        assertThat(IOUtils.toString(test.get(id).toBlocking().first(), Charset.defaultCharset())).isEqualTo("ciao2");
    ...
    public void contentLength() throws Exception {
        ...
        String id = test.postLarge(new ByteArrayInputStream("ciao".getBytes()), new TransferParams(4)).toBlocking().first();
        assertThat(IOUtils.toString(test.get(id).toBlocking().first(), Charset.defaultCharset())).isEqualTo("ciao");
        assertThat(test.putLarge(id, new ByteArrayInputStream("ciao2".getBytes()), new TransferParams(5)).toBlocking().first()).isTrue();
        assertThat(IOUtils.toString(test.get(id).toBlocking().first(), Charset.defaultCharset())).isEqualTo("ciao2");
        assertThat(test.delete(id).toBlocking().first()).isTrue();
    ...
    public void notfound() throws Exception {
        ...
        test.setConf(createConf());
        test.init(Utils.deviceId()).toBlocking().first();
        test.get("sdfsefwf23").toBlocking().first();
